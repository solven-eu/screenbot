...
public class TestIsMenu {
	private static final Logger LOGGER = LoggerFactory.getLogger(TestIsMenu.class);
	...
	public void testIsMenu_isMenu() throws IOException {
		Stream.of(new PathMatchingResourcePatternResolver().getResources("/menu/*")).forEach(resource -> {
			...
			try {
				...
				Assertions.assertThat(VampireSurvivorExpertise.isMenu(img)).as(resource.toString()).isTrue();
			} catch (IOException e) {
	...
	public void testIsMenu_notMenu() throws IOException {
		Stream.of(new PathMatchingResourcePatternResolver().getResources("/otherapp/*")).forEach(resource -> {
			...
			try {
				...
				Assertions.assertThat(VampireSurvivorExpertise.isMenu(img)).as(resource.toString()).isFalse();
			} catch (IOException e) {
				throw new UncheckedIOException(e);
			}
		});
